#!/usr/bin/env node
/**
 * kg-auto-update-cline.cjs — Cline Stop/SessionEnd Hook
 *
 * Cline's hook payload format matches Claude Code (JSON stdin with conversation).
...
function main() {
  try {
    ...
    require('./lib/sqlite-bridge.cjs').bindHookContext(payload, 'cline');
    ...
    process.stderr.write(`[hermit] kg-auto-update-cline: ${newEntities.length} new entities\n`);
    process.exit(0);
