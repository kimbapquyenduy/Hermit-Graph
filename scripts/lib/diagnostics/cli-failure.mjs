import {currentContext,safeContext} from './context.mjs';
...
export function reportCliFailure(error,{component='cli',phase='request'}={}){
 ...
 if(process.env.HERMIT_DIAGNOSTICS!=='off')try{capsule=writeCapsule({directory:resolvePaths().crashSpoolDir,metadata:{code:error?.code,component,phase},context});}catch{}
