...
export class SessionService{
 ...
 lookup(input){const x=this.identity(input);if(!x)return null;return map(this.store.db.prepare('SELECT * FROM runtime_sessions WHERE project_id=? AND agent=? AND upstream_hash=?').get(x.projectId,x.agent,x.key));}
