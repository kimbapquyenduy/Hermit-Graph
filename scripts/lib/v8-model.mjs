...
import { readFileSync, writeFileSync, mkdirSync, renameSync, readdirSync, statSync, mkdtempSync, rmSync, existsSync } from 'node:fs';
...
const transformersEngine = {
  async install({ modelsDir }) {
    ...
    const extractor = await pipeline('feature-extraction', MODEL_ID, { dtype: 'fp32', local_files_only: false, revision: MODEL_REVISION, cache_dir: modelsDir });
...
export class ModelManager {
  ...
  status() {
    try {
      ...
      if (manifest.model !== MODEL_ID || manifest.dimension !== EMBEDDING_DIM || manifest.version !== 1 || manifest.revision !== MODEL_REVISION) throw new Error('incompatible model manifest');
  ...
  async install() {
    ...
    return withMaintenance(join(this.modelsDir,'installation'),async()=>{
      if(this.status().installed)return this.status();
      const staging=mkdtempSync(join(this.modelsDir,'.install-'));
      const staged=new ModelManager({dataDir:staging,engine:this.engine});
      try{
        mkdirSync(staged.modelDir,{recursive:true});
        await this.engine.install({modelDir:staged.modelDir,modelsDir:staged.modelsDir,revision:MODEL_REVISION});
        const files={};
        const walk=dir=>{for(const entry of readdirSync(dir,{withFileTypes:true})){const path=join(dir,entry.name);if(entry.isSymbolicLink())throw new Error('Invalid model symlink');if(entry.isDirectory())walk(path);else if(entry.isFile()&&!entry.name.startsWith('hermit-manifest.'))files[relative(staged.modelDir,path).replaceAll('\\','/')]=sha256(readFileSync(path));}};
        walk(staged.modelDir);
        for(const file of REQUIRED_ARTIFACTS)if(!files[file])throw new Error('installer did not produce required artifact: '+file);
        writeFileSync(staged.manifestPath,JSON.stringify({version:1,model:MODEL_ID,revision:MODEL_REVISION,dimension:EMBEDDING_DIM,files},null,2),{flag:'wx'});
        const valid=staged.status();if(!valid.installed)throw new Error(valid.reason);
        mkdirSync(join(this.modelDir,'..'),{recursive:true});
        if(existsSync(this.modelDir))renameSync(this.modelDir,this.modelDir+'.invalid-'+Date.now());
        renameSync(staged.modelDir,this.modelDir);this.loading=null;return this.status();
  ...
  ensureTable() {
    ...
    this.store.db.prepare('SELECT entity_id FROM semantic_vectors LIMIT 0').all();
  }
  async backfill(options = {}) {
    ...
    for (const entity of entities) {
      ...
      if (old?.content_hash === hash && old.model === MODEL_KEY && old.dimension === EMBEDDING_DIM) continue;
      const vector = await this.embed(text); put.run(entity.id, hash, MODEL_KEY, EMBEDDING_DIM, Buffer.from(vector.buffer, vector.byteOffset, vector.byteLength)); updated++;
    }
  ...
  async search(query, { mode = 'lexical', topK = 10, ...scope } = {}) {
    ...
    try {
      ...
      const entities = this.store.listEntities({ ...scope, lifecycles: ['active'] }).map(entity => {
        const row = get.get(entity.id, MODEL_KEY, EMBEDDING_DIM); if (!row) return null;
