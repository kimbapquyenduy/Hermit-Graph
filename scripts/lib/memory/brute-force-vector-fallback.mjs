 ...
 * Identical return shape to SqliteVecBackend.search: [{id, name, distance}].
 ...
 *   await backend.addAll(entries); // entries: [{id, name, vec}]
...
export class BruteForceVectorBackend extends VectorBackend {
  ...
  constructor(initialEntries = []) {
    ...
    for (const { id, name, vec } of initialEntries) {
      this._validateAndStore(id || name, vec);
    }
  }
   ...
   */
  _validateAndStore(entityId, vec) {
    if (!entityId) throw new Error('BruteForceVectorBackend: entityId is required');
  ...
  async addAll(entries) {
    for (const { id, name, vec } of entries) {
      this._validateAndStore(id || name, vec);
    }
  }
  ...
  async upsert(entityId, vec) {
    this._validateAndStore(entityId, vec);
  }
  ...
  async search(queryVec, topK = 10) {
    ...
    for (const [id, vec] of this._store) {
      scored.push({ id, name: id, distance: cosineDistance(queryVec, vec) });
    }
