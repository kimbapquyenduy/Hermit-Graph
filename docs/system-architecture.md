...
Diagnostics wrap the MCP protocol handler once, including SDK validation and returned errors. Safe metadata enters a bounded SQLite ledger. CLI/startup failures use bounded capsules, imported idempotently at boot. Incident promotion requires verified internal faults, complete human evidence and matching project/domain identity.
